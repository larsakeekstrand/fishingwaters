...
describe('Map', () => {
  ...
  it('renders map with all features when no species filter is applied', () => {
    ...
    render(<Map data={data} filteredSpecies={new Set()} onLakeSelect={jest.fn()} />);
  ...
  it('filters features based on selected species', () => {
    ...
    render(<Map data={data} filteredSpecies={new Set(['Gädda'])} onLakeSelect={jest.fn()} />);
  ...
  it('renders all markers in red color when filter is applied', () => {
    const features = [
      createMockFeature('Lake 1', [18.0579, 59.3293], ['Gädda', 'Abborre']),
      createMockFeature('Lake 2', [17.0579, 58.3293], ['Gös', 'Abborre'])
    ];
    ...
    const markers = screen.getAllByTestId('circle-marker');
    expect(markers).toHaveLength(1); // Only one lake has Gädda

    // Since we're using mocked components, we can't directly test the color
    // but this test ensures the code path is covered for red dots
  });
